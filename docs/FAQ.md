...
* предложите вопрос для FAQ раздела в [ZEEKR Club в
  Telegram](https://t.me/zeekrclub/158198). Пожалуйста, предерживайтесь [правил клуба](rules.md).
...
Далее заряжаем 12В АКБ или [прикуриваем](FAQ.md#mozhno-li-prikurivat-gde-12v-akb-) от источника питания до 14В 10А..
...
??? note "Чуть подробнее по 007"

    Ситуация:<br>
    Оставил машину на зарядке от обычной ракетки на 5 дней. В результате батарею зарядил, а вот 12В аккумулятор сел ( видимо при зарядке работает какая-то Электроника от 12В сети).<br>
    Решение:<br>
    Шаг первый - попасть в машину.<br>
    Под правой фарой в бампере лючек. В нем 2 провода (красный и черный). На них нужно подать питание 12В. Я это сделал прикурив от машины с ДВС (естественно не заведенной). После          подключения прикладывайте карточку к арке - откроется водительская дверь. При этом машина останется дохлой.<br>
    Шаг второй - раскирпичивание<br>
    Открываете капот. В нише переднего багажника лючек - открываете. В нем, в нише с правой стороны плюсовая Клема 12В сети. Минус любой открытый метал ( я кинул провод на петлю            капота. После подключения внешнего аккумулятора ( не заводя машину с ДВС) к клемам Zeekr и ключом снимаете с охраны (кнопка с открытым замком на пульте). Машина оживает!<br>
    Потом прокатиться минут 10, чтобы зарядить аккумулятор.
...
!!! warning
    Не используйте аккумуляторы с напряжением выше 12В. Прикуривать можно от источника питания до 14В 10А.
...
1. Отключите питание батареи высокого напряжения (раздел "Защита и безопасность, в самом низу последний пункт). Убедитесь, что автомобиль находится на безопасной площадке и вы полностью отключили высоковольтную систему.
1. Откройте капот, подождите 5 минут. Во время этого процесса не открывайте и не закрывайте двери.
1. Снемите крышку и разомкните "петлю", подождите 10 секунд
1. Отключите основную батарею 12В (можно просто снять клемму "-")
1. Собирать в таком порядке: соединить петлю ([детальное
...
!!! warning
    ...
    (jack mode) в настройках. [Как включать смотрим тут.](service.md#shinomontazh)
    Если этого не сделать - машина опустится и загорится ошибка пневмы. Нужно поднять
    опустить заднюю подвеску кнопками в багажнике, либо проехать несколько метров и ошибка пропадет.
...
"вернуться" самостоятельно -- [смотреть тут](https://t.me/zeekrclub_tech/77972).
...
## Передача БТ ключей в совместный доступ

БТ ключи можно передать в управление на аккаунт с совместным доступом? В приложении с МА зайдите в ключи, в разделе с БТ ключами нажмите на черную кнопку (Запрос статуса активации) ключа который хотите расшарить. Появится окно где предложат ввести номер телефона. Вводите туда свой номер из eSender на который получен совместный доступ и ключ подвяжется к вашему аккаунту совместного доступа. Теперь если вы откроете машину этим БТ ключом автораизация в машине будет с вашим аккаунтом. Во время производимых действий нужно находиться в машине!

??? info "Zeekr app - скрин с настройкой"
    ![2024-03-13-key-sharing](img/2024-03-13-key-sharing.jpeg){ loading=lazy }
...
## Как выключить режим комфортной посадки?

Что бы автомобиль не опускался в самое нижнее положение, когда переключаете в P (паркинг).
Нужно отключить соответстующий пункт в меню вождения. Данный режим не влияет на Удобный вход

??? info "Zeekr 001 - фото меню"
    ![2024-08-09-exit-comfort-park-001](img/2024-08-09-exit-comfort-park-001.jpg){ loading=lazy } 


## Что такое Удобный вход, как им пользоваться?

При переводе в P (паркинг) сиденье отодвигается назад, для удобного выхода из автомобиля

??? info "Zeekr 001 - фото меню"
    ![2024-08-09-seat-back-001](img/2024-08-09-seat-back-001.jpeg){ loading=lazy } 


## Поведение зеркал при закрытии авто

Что бы зеркала складывались или наоборот оставались в разложенном состоянии когда авто закрыватеся - установите соответствующий переключатель в настройках зеркал.

??? info "Zeekr 001 - фото меню"
    ![2024-08-09-mirror-close](img/2024-08-09-mirror-close.jpeg){ loading=lazy } 


## Регулеровка руля

??? info "Zeekr 001 - фото меню"
    ![2024-08-09-wheel-adjust](img/2024-08-09-wheel-adjust.jpeg){ loading=lazy } 
